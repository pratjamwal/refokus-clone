...
function Navbar() {
  return (
    <div className='max-w-screen-xl mx-auto py-6 flex items-center justify-between  border-b-[1px] border-zinc-700'>
       ...
       <img
         src="https://cdn.prod.website-files.com/62df9251ae9124d9d726bcb9/62dfc1419247bccdd5a2dd1b_Tools.svg" alt="" 
         />
                ...
                elem.length === 0 ? (<span key={index} className='w-[2px] h-7 bg-zinc-700'></span>
                   ...
                   key={index}
                    className='font-regular text-sm flex items-center gap-1 text-white'
                     href='#'>
                    {index === 1 && (
                      <span style={{boxShadow:"0 0 0.45em #00FF19"}}
                       className='inline-block w-1 h-1 rounded-full bg-green-500'></span>
                      )}
                    {elem}
                </a>)
                ))}
